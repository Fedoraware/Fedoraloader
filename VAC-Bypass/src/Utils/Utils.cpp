...
void Utils::HookImport(LPCSTR moduleName, LPCSTR importModuleName, LPCSTR functionName, PVOID fun)
{
	const auto module = reinterpret_cast<PBYTE>(GetModuleHandleA(moduleName));
	...
	if (module)
	{
		...
		for (PIMAGE_IMPORT_DESCRIPTOR import = imports; import->Name; import++)
		{
			...
			for (auto originalFirstThunk = reinterpret_cast<PIMAGE_THUNK_DATA>(module + import->OriginalFirstThunk), firstThunk = reinterpret_cast<PIMAGE_THUNK_DATA>(module + import->FirstThunk); originalFirstThunk->u1.
			     AddressOfData;
